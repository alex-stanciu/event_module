...
class EventRestResource extends ResourceBase {
   ...
   *
   * Provides event entities that fall between the start and end dates.
   * If no start date is specified, the current date is used.
   *
  ...
  public function get() {
    ...
    // Create cacheable metadata to make sure we vary the response
    // based on the url query arguments.
  ...
  /**
   * Provides a start date based on the current request.
   *
   * If the argument is missing, the current date is returned.
   *
  ...
  protected function getStartDate() {
    ...
    if (!empty($start)) {
      // Since we are using user-provided data, we must expect an exception,
      // as the data could be invalid.
      try {
